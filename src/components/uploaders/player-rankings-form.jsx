  ...
  const onChange = (option, ranking=null) => {
    ...
    if(!ranking) {
      const updateRankings = (type) => 
        (rankingList || []).filter(r => !(type || []).some(({ranking}) => r === ranking))
      v === "Batsman" 
    ...
    return ranking 
      ? setRankingDefaultValue(v)
      : setTypeDefaultValue(v);
  }

  const handleSumbit = (e) => {
    e.preventDefault();
    postRankings({fullname, ranking: rankingDefaultValue}, typeDefaultValue)
      .then(() => {
        setFullname("");
