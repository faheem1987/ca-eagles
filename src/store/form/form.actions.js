...
export const postRankings = (ranking, type) => (dispatch, getState, { getFirestore }) => {
  dispatch({ type: types.POST_RANKING_REQUEST });
  const firestore = getFirestore();
  const collectionType = type === 'Bowler' ? 'bowlersRanking' : 'battersRanking';
  return firestore.collection(collectionType)
...
export const uploadImage = (playerBio) => (dispatch, getState, { getFirebase, getFirestore }) => {
  ...
  return uploadTask.on(
    ...
    () => firebase.storage()
      ...
      .then((url) => {
        ...
        const DOB = formatedDate(playerBio.DOB);
        return firestore.collection('playerInfo')
          .doc(minifiedName).set({
            url, ...playerBio, DOB, createdAt: new Date(),
          });
      }),
  );
};

export const postMatchResult = (result) => (dispatch, getState, { getFirestore }) => {
  ...
  return firestore.collection('winter2520202021')
    .add({ ...result, matchDate, createdAt: new Date() })
    .then((response) => dispatch({ type: types.POST_MATCH_RESULT_SUCCESS, response }))
